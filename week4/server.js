  ...
  if (req.url === "/api/credit-package" && req.method === "GET") {
    ...
    req.on("end", async () => {
      try {
        ...
        // console.log(isUndefined(data.name))
        if (isUndefined(data.name) || isNotValidSting(data.name) || //檢查欄位是否有填寫 或填寫是否正確 防呆裝置
        ...
        const newPackage = await creditPackageRepo.create({ //新增一筆資料
          name: data.name,
          credit_amount: data.credit_amount,
          price: data.price
        })
        const result = await creditPackageRepo.save(newPackage) //儲存到資料庫
        res.writeHead(200, headers)
        res.write(JSON.stringify({
          status: "success",
          data: result
    ...
    try {
      const skillRepo = await AppDataSource.getRepository("Skill").find({
    ...
    req.on("end", async () => {
      try {
        ...
        // console.log(isUndefined(data.name))
        if (isUndefined(data.name) || isNotValidSting(data.name) || //檢查欄位是否有填寫 或填寫是否正確 防呆裝置
                isUndefined(data.createdAt) || isNotValidInteger(data.createdAt)) {
          res.writeHead(400, headers)
          res.write(JSON.stringify({
            status: "failed",
            message: "欄位未填寫正確"
        ...
        const newPackage = await skillRepo.create({ //新增一筆資料
          name: data.name,
          credit_amount: data.credit_amount,
          price: data.price
        })
        const result = await skillRepo.save(newPackage) //儲存到資料庫
        res.writeHead(200, headers)
        res.write(JSON.stringify({
          status: "success",
          data: result
